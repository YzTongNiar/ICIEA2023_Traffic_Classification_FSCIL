...
from utils import MetricLogger, SoftTarget, build_dataset
...
if __name__ == "__main__":
    ...
    for task_id, dataset_train in enumerate(scenario_train):
        ...
        optimizer = torch.optim.SGD(model_without_ddp.parameters(), lr=args.lr)
        ...
        for epoch in range(args.num_epochs):
            ...
            print(f"train states: epoch :[{epoch+1}/{args.num_epochs}] {metric_logger}")

            if (epoch+1) % args.eval_every_epoch == 0:
                eval(model, val_loader)

        model_without_ddp.after_model_adaption(args.increment_per_task[task_id], args)
